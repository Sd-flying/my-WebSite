...
<script>
  export default {
    ...
    props: {
      ...
      lineColor: {
        type: String,
        default: 'rgba(5,5,5,0.9)' // 线的颜色
      },
      roundColor: {
        type: String,
        default: 'rgba(240,240,124,0.7)' // 星星的颜色
      },
    },
    data() {
      return {
    ...
    mounted() {
      const canvasDom = this.$refs.canvas // 取画布的高宽来设置显示分辨率
        ...
        for (let i = 0; i < this.point; i++) {
          for (let j = 0; j < this.point; j++) {
            if (i + j < this.point) {
              ...
              if (lineOpacity > 0) {
                this.drawLine(this.context, circleArr[i].x, circleArr[i].y, circleArr[i + j].x, circleArr[i + j].y, lineOpacity,)
              }
            }
          }
        }
...
<style>
  .starry-sky {
    ...
    height: 692.5px !important;
    /*background-color: #79F0E5;*/
  }
